...
import { edifSinUnid, maderaInsuf, maxEdif, panIns, piedraInsuf, unidExed, unidIns, unidNegat } from "@/helpers/error"
...
export async function getOneBuildingsByUserId(usuarioId: string, id:string){
    console.log("usuarioId", usuarioId)
    let user = usuarioId
    try {

        // Buscar todos los edificios creados por el usuario con el ID proporcionado
        const building = await prisma.userEdificios.findUnique({
            where: {
                id: id, // Utilizar el `userId` proporcionado en la llamada
            },
            include: {
                edificio: {
                    select: {
        ...
        // Mapeamos los resultados para que tengan el formato deseado
        let data
        if(building && building.edificioId){
            data = {
                // Utilizar el id de la relación UserEdificios
            id: id,
            x: Number(building.posicion_x),
            y: Number(building.posicion_y),
            type: String(building.edificio.name), // Usar el nombre del edificio como tipo
            costo: Number(building.edificio.costo),
            ancho: Number(building.edificio.ancho),
            largo: Number(building.edificio.largo),
            nivel: Number(building.nivel),
            edificioId: String(building.edificioId),// para la imagen
            cantidad: Number(building.edificio.cantidad)
            }
        }
